...
def train(
    ...
    for batch in tqdm(data_loader, total=len(data_loader), leave=False):
        ...
        mol_batch, features_batch, target_batch, mask_batch, atom_descriptors_batch, atom_features_batch, bond_descriptors_batch, bond_features_batch, constraints_batch, data_weights_batch = \
            ...
            batch.atom_features(), batch.bond_descriptors(
            ), batch.bond_features(), batch.constraints(), batch.data_weights()
        ...
        if model.is_atom_bond_targets:
            ...
            for i in range(len(args.atom_targets)):
                if not args.atom_constraints[i]:
                    ...
                    constraints_batch[ind] = torch.tensor(
                        constraints_batch[ind]).to(args.device)
                ind += 1
            ...
            for i in range(len(args.bond_targets)):
                if args.adding_bond_types and atom_bond_scaler is not None:
                    mean, std = atom_bond_scaler.means[i+len(
                        args.atom_targets)][0], atom_bond_scaler.stds[i+len(args.atom_targets)][0]
            ...
            if args.target_weights is not None:
                target_weights = torch.tensor(
                    args.target_weights).unsqueeze(0)  # shape(1,tasks)
            else:
                target_weights = torch.ones(targets.shape[1]).unsqueeze(0)
            data_weights = torch.tensor(
                data_weights_batch).unsqueeze(1)  # shape(batch,1)
        ...
        if model.is_atom_bond_targets:
            ...
            for target, pred, target_weight, data_weight, mask in zip(targets, preds, target_weights, data_weights, masks):
                if args.loss_function == "mcc" and args.dataset_type == "classification":
                    loss = loss_func(pred, target, data_weight,
                                     mask) * target_weight.squeeze(0)
                elif args.loss_function == "bounded_mse":
                    raise ValueError(
                        f'Loss function "{args.loss_function}" is not supported with dataset type {args.dataset_type} in atomic/bond properties prediction.')
                elif args.loss_function in ["binary_cross_entropy", "mse", "mve"]:
                    loss = loss_func(pred, target) * \
                        target_weight * data_weight * mask
                elif args.loss_function == "evidential":
                    loss = loss_func(
                        pred, target, args.evidential_regularization) * target_weight * data_weight * mask
                    ...
                    loss = loss_func(pred, target, quantiles_tensor) * \
                        target_weight * data_weight * mask
                else:
                    raise ValueError(
            ...
            if args.loss_function == "mcc" and args.dataset_type == "classification":
                loss = loss_func(preds, targets, data_weights,
                                 masks) * target_weights.squeeze(0)
            elif args.loss_function == "mcc":  # multiclass dataset type
                targets = targets.long()
                target_losses = []
                for target_index in range(preds.size(1)):
                    target_loss = loss_func(
                        preds[:, target_index, :], targets[:, target_index], data_weights, masks[:, target_index]).unsqueeze(0)
                ...
                if args.loss_function == "dirichlet":
                    loss = loss_func(
                        preds, targets, args.evidential_regularization) * target_weights * data_weights * masks
                else:
                    target_losses = []
                    for target_index in range(preds.size(1)):
                        target_loss = loss_func(
                            preds[:, target_index, :], targets[:, target_index]).unsqueeze(1)
                        target_losses.append(target_loss)
                    loss = torch.cat(target_losses, dim=1).to(
                        torch_device) * target_weights * data_weights * masks
            elif args.dataset_type == "spectra":
                loss = loss_func(preds, targets, masks) * \
                    target_weights * data_weights * masks
            elif args.loss_function == "bounded_mse":
                loss = loss_func(preds, targets, lt_target_batch,
                                 gt_target_batch) * target_weights * data_weights * masks
            elif args.loss_function == "evidential":
                ...
                quantiles_tensor = torch.tensor(
                    args.quantiles, device=torch_device)
                loss = loss_func(preds, targets, quantiles_tensor) * \
                    target_weights * data_weights * masks
            else:
